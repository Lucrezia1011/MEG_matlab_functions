function [tfced] = matlab_tfce_transform_forbts(img,H,E,C,dh, varargin)
...
for rep = 1:2
    ...
    tfce_temp = NaN(size(img));
    tfce_temp(:) = vals.*dh;
    if rep == 1
        tfced = tfce_temp;
    else
        tfced = tfced - tfce_temp;
    end
    
end
tfced = tfced(:);
if nargin > 5
tfced = tfced(gridall==1);
end
if nargin == 8
    ii = varargin{3};
    tfced = tfced(ii);
